# Task 04 - Baseline run matrix

## Objective

Define a run matrix for target model only, target + FlashAttention, target + TurboQuant, and target + FlashAttention + TurboQuant.
...
```text
phase-1: task 04 - baseline run matrix
```
